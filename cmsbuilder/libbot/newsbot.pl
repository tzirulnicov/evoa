...
#@{$handle->{links}}='/film/man_of_the_year.html';
	...
	DEBUG=>$DEBUG)){
	 ...
	 # Проверяем, имеется ли такой фильм в базе
         $ds=$db_handleAr[1]->prepare('SELECT * from dbo_bQuestion
		where question=?');#,photo
         $ds->execute($body);
...
sub insert_record{
   ...
   $dbs=$db_handleAr[1]->prepare('SELECT ID from dbo_bTheme where
	ID=?');
   $dbs->execute($papa_id);
   die "Can't find theme for this item!" if !($catdir_id=$dbs->fetchrow_hashref);
   $dbs=$db_handleAr[1]->prepare('INSERT into dbo_bQuestion (CTS,OWNER,
	PAPA_CLASS,PAPA_ID,photo,photobig2,photobig3,price,start,end,username,
	question,city,birth_days,lifetime,torg,tel) values (
