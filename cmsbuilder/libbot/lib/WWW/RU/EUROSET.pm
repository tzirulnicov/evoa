...
sub _get_url_by_name{
   my $self = shift;
   my $name = shift;
   my $url_name = $name;
   $url_name=~s/\s/\+/g;
 	 $url = qq~/search/?filter=all&q=$url_name&filter[]=products~;
   if (!$self->{sock}->get($url,NoEncoding=>1)){
      $self->{'errmsg'}="Error in WWW::RU::EUROSET->_get_url_by_name (get 'http://www.euroset.ru/$url')".$self->{sock}->{errmsg};
      return 0;
   }
   my $html=$self->{sock}->{'net_http_content'};
	 if ( $self->{sock}->{'net_http_content'}=~m|<a href="(.*?)">$name| ){
	 	  return $1;
	 }
	 $self->{errmsg}='Error in WWW::RU::EUROSET->_get_url_by_name (can\'t find item in search result)';
	 return 0;
}
...
#$url='/catalog/mobile/phones/nokia/-/18299/';
   ...
   NEXT_LOOP:
   print "url: $url\n" if $arg{'DEBUG'};
 	 $url = $self->_get_url_by_name($arg{BY_NAME}) if $arg{BY_NAME};
   if (!$self->{sock}->get($url,NoEncoding=>1)){
...
#if ($html=~m|<meta itemprop="image" content="([^"]+)"|){#"
#   warn "image: $1";
#}
if ($html=~m|<div id\="scrollcontent">(.*?)</div>|s){
   @{$self->{content_images}}=$1=~m|<a rel\="([^"]+)"|g;#"
}
   foreach my $image(@{$self->{'content_images'}}){
  	 my $filename = $image;
  	 $filename=~s/(\-\d+)_448x460.*?(\.[a-z]+)$/\1\2/;
  	 $filename=~s/\/upload\/iblock\/\w+\///;
		 if (-e $arg{SAVE_IMG_PATH}.$filename){
  	    warn "Photo $filename already downloaded!";
  	    next;
  	 };
  	 my $img = get('http://euroset.ru'.$image);
     unless ($img){
     	  $image='';
     	  next;
  	    warn "Can't download $image!";
  	 }
warn $path.$filename;
  	 open(FILE,'>'.$arg{SAVE_IMG_PATH}.$filename) or die $!;
  	 binmode(FILE);
  	 print FILE $img;
  	 close(FILE);
#  	 $count++;
