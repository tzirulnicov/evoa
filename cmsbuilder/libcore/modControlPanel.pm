...
sub _simplem_menu
{
	...
	'cpanel_drop_cache'	=>{-name=>'Удалить кеш страниц',-icon=>'icons/install.gif'}
}
...
#(c)tz
sub cpanel_drop_cache{
	my $o=shift;
	my $r=shift;
	open(PROG,$CMSBuilder::Config::path_cmsb."/libbot/rm.pl ".$CMSBuilder::Config::path_tmp."/cache/|") or die $!;
	print $_.'!' while <PROG>;
	close(PROG);
	print "\n<br>OK";
	`touch $CMSBuilder::Config::path_cmsb/tmp/start_cache`;
}
