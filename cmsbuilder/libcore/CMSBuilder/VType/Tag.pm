...
sub aedit
{
	...
	my @alltags = modTags->new(1)->get_all();
	
	map { push @stags, $_->name() if $r->{$_->myurl} } @alltags;
	
	$val = join ', ', @stags;
	
	map { $_->elem_relation_del($obj,'tag') } modTags->new(1)->get_all();
