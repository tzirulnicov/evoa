...
sub site_template_object
{
	my $o = shift;
	my $r = shift;
#return TextTemplate->first($1) if ($o->myurl=~/(CatBasket|CatWareSimple)/ &&
#	!($o->myurl=~/(CatWareSimple)/ && $ENV{'SERVER_NAME'} eq $CMSBuilder::Config::server_main));
#print ( $o->{template}?"OK!!!".$o->myurl:":(");
