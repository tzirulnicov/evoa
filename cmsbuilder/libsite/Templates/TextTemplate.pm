...
#print $k.'='.$ENV{$k}.'<br>';
#}
# my $str = $CMSBuilder::DBI::dbh->prepare('SELECT template from dbo_modSite);
#		         $str->execute(@_);	 
...
$CMSBuilder::site_id=$k->{ID};
	 ...
	 next if (($obj_name eq 'CatBasket' && ($k2->{name} ne 'Корзина')) || 
		($obj_name eq 'CatWareSimple' && ($k2->{name} ne 'Карточка товара')));
	 return $k2;
      }
   }
   return $k->{template};
